...
double error = 0;
double speed = 30;
double segment_threshold = 300; //// Set segmentation constants
double segment_angle = 45;
...
int main(int argc, char **argv) {
  ...
  while (robot->step(timeStep) != -1) {
    ...
    double dist_left = sensor_left->getValue();
    // double dist_mid = sensor_mid->getValue();
    double dist_right = sensor_right->getValue();    
    ...
    if (dist_left > 800 && dist_right > 800) {
    ...
    } else if (dist_left < 800 && dist_right < 800) {
      leftMotor1->setVelocity(0.0);
      leftMotor2->setVelocity(0.0);
      rightMotor1->setVelocity(0.0);
      rightMotor2->setVelocity(0.0);
    } else if (dist_left < 800) {
      leftMotor1->setVelocity(MAX_SPEED/4);
      leftMotor2->setVelocity(MAX_SPEED/4);
      rightMotor1->setVelocity(MAX_SPEED);
      rightMotor2->setVelocity(MAX_SPEED);
    } else if (dist_right < 800) {
      leftMotor1->setVelocity(MAX_SPEED);
      leftMotor2->setVelocity(MAX_SPEED);
      rightMotor1->setVelocity(MAX_SPEED/4);
      rightMotor2->setVelocity(MAX_SPEED/4);
    }
  };
